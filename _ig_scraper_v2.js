(async function() {
  'use strict';
  const posts = new Map(); // shortcode -> clean url
...
<div style="position:fixed;bottom:20px;right:20px;z-index:99999;
     ...
     min-width:300px;max-width:400px;border:1px solid #e1306c;
     max-height:90vh;overflow-y:auto;">
  <div style="font-weight:700;color:#e1306c;margin-bottom:8px;font-size:15px;">
    IG Scraper v2 <span style="color:#888;font-weight:400;font-size:12px;">(feed + reels)</span>
  </div>
  <div style="margin:8px 0;">
    <span id="__igc" style="font-size:20px;font-weight:700;">0</span>
    <span style="color:#aaa;">posts</span>
  </div>
  <div id="__igs" style="color:#aaa;font-size:12px;margin-bottom:6px;">Starting...</div>
  <div style="display:flex;gap:6px;flex-wrap:wrap;">
    <button id="__ig_stop" style="flex:1;padding:6px 0;border:1px solid #e1306c;
           background:transparent;color:#e1306c;border-radius:6px;cursor:pointer;
           font:inherit;min-width:80px;">Stop & Save</button>
    <button id="__ig_copy" style="padding:6px 10px;border:1px solid #555;
           background:transparent;color:#aaa;border-radius:6px;cursor:pointer;
           font:inherit;">Copy URLs</button>
  </div>
  <div id="__ig_log" style="margin-top:6px;font-size:11px;color:#666;
       max-height:150px;overflow-y:auto;border-top:1px solid #333;padding-top:4px;"></div>
</div>`.trim();
  document.body.appendChild(panel);

  const $ = id => document.getElementById(id);
  const log = msg => { const e = $('__ig_log'); if (e) { const d = document.createElement('div'); d.textContent = msg; e.appendChild(d); e.scrollTop = e.scrollHeight; } };
  const updateCount = () => { const c = $('__igc'); if (c) c.textContent = posts.size; };
  const status = msg => { const s = $('__igs'); if (s) s.textContent = msg; };
  ...
  function collectFromDOM() {
    ...
    document.querySelectorAll('a[href*="/reel/"], a[href*="/p/"]').forEach(a => {
      ...
      const m = clean.match(/\/(reel|p)\/([^/]+)/);
      const key = m ? m[2] : clean;
      if (!posts.has(key)) { posts.set(key, clean); added++; }
    });
  ...
  $('__ig_stop').onclick = () => { stopped = true; if (!finished) finish(); };
  $('__ig_copy').onclick = () => {
    ...
    for (let i = 0; i < 300 && !stopped; i++) {
      ...
      if (added === 0) {
        ...
        if (empty >= 8) {
          ...
          if (collectFromDOM() === 0) {
            // Scroll UP then DOWN to trigger different API page
  ...
  // ── Pass 1: normal scroll ──
  await scrollPass('Pass 1/2');

  // ── Pass 2: reset and re-scroll ──
  ...
  status('Done - '+posts.size+' posts. Click Stop & Save to download.');

  async function finish() {
    if (finished) return; // prevent double-save
    finished = true;
    observer.disconnect();
    ...
    if (titleMatch) name = titleMatch[1];
      ...
      if (pathMatch && !['reels','p','reel','explore'].includes(pathMatch[1])) name = pathMatch[1];
    }
    const ts = new Date().toISOString().replace(/[:.]/g,'-').slice(0,-5);
    const blob = new Blob([urls.join('\n')], {type:'text/plain'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = name+'_videos_'+ts+'.txt';
    a.click();
    URL.revokeObjectURL(a.href);
    log('Downloaded: '+name+'_videos_'+ts+'.txt');
  }

  // Always auto-save when scrolling finishes
